...
- `SG-controls`: Expanded controls offer Pause, Resume, Edit budget, and Clear as
  ...
  Pause is recorded as a clean interruption. Between continuation turns, Pause
  prevents further work without trying to interrupt a finished turn.
...
- `SG-worker`: Running changes go through the owning worker and require
  acknowledgement. The worker rechecks goal state when applying queued controls;
  it rejects completed goals and leaves ordinary messages with already-paused
  goals running. Idle changes use the native goal API without resuming the thread.
  Starting and resuming use a detached worker that follows native continuation
  turns, with the session's model, effort, execution permissions,
  and instructions. Ordinary composer messages pause any saved active goal
  before loading the thread; goal continuation requires an explicit Resume.
  Messages between continuations wait in the worker without delaying goal
  controls. If the goal ends first, undelivered messages use the existing
...
- `SG-persistence`: Goal-controlled sessions keep a stable Codex SQLite home so
  ...
  available when that turn finishes. Forced termination before that first
  preservation can lose an agent-created goal in a temporary home. Historical
  event logs and old pooled databases are not searched to recover goals.
  Workers acquire the session lease before choosing a database and retain it
  through first-time goal preservation. Idle controls acquire that lease before
  reading or creating the goal home. A follow-up
  worker waits up to ten seconds for the previous lease to be released before
  failing. Once a cleared goal's Codex process closes, its databases are removed;
  later workers reuse the empty dedicated home.
  The panel treats a database whose schema is still initializing or which is
  removed during Clear as no goal.
- `SG-live`: Live updates preserve expanded state and unsaved dialog and composer
  ...
  Composer drafts also survive the current turn ending after Clear.
