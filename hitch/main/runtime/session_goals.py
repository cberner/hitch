...
def acquire_home(thread_id: str, *, wait_seconds: float = 0) -> codex_pool.WorkerSqliteHome:
...
def _stored_goal(database: Path, thread_id: str) -> dict[str, Any] | None:
    ...
    try:
        with contextlib.closing(sqlite3.connect(database.as_uri() + "?mode=ro", uri=True)) as db:
            ...
            # Codex creates the file before applying its schema.
            if not db.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='thread_goals'").fetchone():
                return None
...
def preserve_worker_goal(thread_id: str, source_home: str) -> None:
    """Keep agent-created goals while the worker still holds the session lease."""
    home = sqlite_home(thread_id)
...
class GoalTurn(TurnHandle):
    ...
    def steer(self, input: Input | str) -> TurnSteerResponse:
        turn_id = self.state.current_turn()
        if turn_id is None:
            raise ValueError("The goal has no running turn.")
