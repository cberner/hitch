...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        session_lease: WorkerSqliteHome | None = None
        ...
        try:
            if not getattr(settings, "TESTING", False):
                # Home selection must wait for the previous worker's goal preservation.
                session_lease = session_goals.acquire_home(instance.thread_id, wait_seconds=10)
                if goal_home.is_dir():
                    sqlite_lease = session_lease
            ...
            if session_lease is not None and session_lease is not sqlite_lease:
...
def _run_turn(
    *,
    ...
    def _configure(codex: CodexApi) -> None:
        ...
        goal = None
        if not resume_goal:
            goal = cast(dict[str, Any], codex._client._request_raw(
                "thread/goal/get", {"threadId": instance.thread_id},
            )).get("goal")
        if resume_goal or (goal and goal.get("status") == "active"):
            # Loading an active goal starts work. Activation must wait for an
            # explicit goal resume with its continuation handle subscribed.
...
def _forward_steer_requests(
    *,
    ...
    while not stop.is_set():
        if isinstance(turn, session_goals.GoalTurn):
            turn_id = turn.state.current_turn()
            if turn_id is not None and turn_id != model_turn_id:
                model_turn_id = turn_id
                # Keep continuation settings and acknowledged user changes ordered.
                if selected := session_model_override(instance.thread_id):
                    _apply_turn_model_settings(turn, instance, {"model": selected[0], "effort": selected[1]})
        ...
        if (
            not isinstance(turn, session_goals.GoalTurn)
            or turn.state.current_turn() is not None or turn.state.is_finished()
        ):
            for request in pending_steers:
                _deliver_steer_request(turn, instance, control_path, request)
    ...
    *,
    ...
    for raw in complete.split(b"\n"):
        ...
        if request.get("op") == "goal":
            ...
            try:
                ...
                if change["action"] != "clear":
                    goal = cast(dict[str, Any], turn._client._request_raw(
                        "thread/goal/get", {"threadId": turn.thread_id},
                    )).get("goal")
                    if not goal:
                ...
                if change["action"] == "pause" and (
                    isinstance(turn, session_goals.GoalTurn) or goal and goal.get("status") == "active"
                ):
        ...
        if pending_steers is not None:
            pending_steers.append(request)
        else:
            _deliver_steer_request(turn, instance, control_path, request)
    return control_offset + len(complete)


def _deliver_steer_request(
    turn: TurnHandle, instance: CodexInstance, control_path: Path, request: dict[str, Any],
) -> None:
...
def _apply_turn_model_settings(
    ...
    if not isinstance(model, str) or not model or not isinstance(effort, str) or not effort:
        return False
    try:
        turn_id = turn.state.current_turn() if isinstance(turn, session_goals.GoalTurn) else turn.id
        if turn_id is None:
            return False
