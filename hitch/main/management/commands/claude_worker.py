...
from hitch.main.runtime import claude_translate, claude_usage
from hitch.main.runtime.claude_tools import (
    PROPOSE_SESSION_TOOL_NAME,
    build_hitch_mcp_server,
)
...
class _TurnRunner:
    ...
    def _record_user_turn_with_image(self, text: str) -> None:
        ...
        Used by the initial turn, whose receive loop has not started yet, so
        writing the entry and arming suppression together here is safe.
        """
        self._write_user_image_event(text)
        self._translator.suppress_next_user_prompt()

    def _write_user_image_event(self, text: str) -> None:
        """Emit the synthetic ``userMessage`` (text + ``[image]`` marker) for an
        image turn the SDK echo can't represent."""
    ...
    def _drain_once(self, control_path: Path, offset: int) -> int:
        ...
        for raw in complete.splitlines():
            text, image_paths, steer_id = _steer_request(raw)
            if (text or image_paths) and self._client is not None and self._loop is not None:
                ...
                future = asyncio.run_coroutine_threadsafe(
                    self._send_steered_query(text, image_paths, steer_id), self._loop
                )
                future.add_done_callback(self._steer_query_done)
        return offset + len(complete)

    async def _send_steered_query(
        self, text: str, image_paths: Any, steer_id: str
    ) -> None:
        ...
        Writes a ``steer_ack`` once delivery is known (like ``codex_worker``) so
        the requesting POST (``codex_pool._await_steer_ack``) redirects to the
        session immediately instead of stalling for the full timeout and then
        re-spawning the already-delivered prompt as a duplicate follow-up.
        """
        client = self._client
        if client is None:
            # The registered response can never arrive; release it so the receive
            # loop can tear down instead of awaiting forever.
            discard_input_attachment_paths(self._instance, image_paths)
            self._ack_steer(steer_id, delivered=False)
            self._add_outstanding(-1)
            return
        query_input = _build_query_input(text, image_paths)
        has_image = not isinstance(query_input, str)
        if has_image:
            # Arm echo suppression BEFORE awaiting the query: this coroutine runs
            # concurrently with the receive loop and ``await client.query`` yields
            # to it, so a later arm would let the loop translate the (image-
            # stripped) text-only echo before suppression is set. But DEFER writing
            # the synthetic ``[image]`` entry until delivery succeeds -- a rejected
            # steer is re-delivered as a fresh follow-up (via the False ack below),
            # and writing it up front would orphan a user entry the model never saw
            # (a duplicate/misleading prompt in the transcript and turn counts).
            self._translator.suppress_next_user_prompt()
        try:
            await client.query(query_input)
        except BaseException:
            if has_image:
                # The query never ran, so no echo will arrive to consume the
                # suppression; release it so it can't swallow a later user prompt.
                self._translator.cancel_pending_user_prompt_suppression()
            # Undelivered: release this steer's duplicated image attachments from
            # the instance's ledger (and delete them) so repeated rejected image
            # steers don't exhaust the per-instance attachment limit -- mirroring
            # the Codex worker's discard on a failed steer. (No-op without images.)
            discard_input_attachment_paths(self._instance, image_paths)
            self._ack_steer(steer_id, delivered=False)
            raise
        if has_image:
            # Delivered: record the user's image turn now, before the receive loop
            # processes (and suppresses) the matching SDK echo.
            self._write_user_image_event(text)
        self._ack_steer(steer_id, delivered=True)

    def _ack_steer(self, steer_id: str, *, delivered: bool) -> None:
        """Append the steer-delivery outcome to the control file (no-op for an
        empty id), so a waiting ``steer_instance`` POST stops polling."""
        _append_steer_ack(
            control_path_for(self._instance), steer_id, delivered=delivered
        )
...
def _steer_request(raw: bytes) -> tuple[str, list[str], str]:
    """Parse one steer control line into ``(text, image_paths, steer_id)``.

    ``steer_ack`` records the worker itself appends to the same control file are
    skipped here (``op != "steer"``), so the drain never re-processes them.
    """
    try:
    ...
    steer_id = request.get("id")
    return text, image_paths, steer_id if isinstance(steer_id, str) else ""
