...
class SpawnNewSessionTests(TestCase):
    ...
    def test_app_server_ignores_unusable_system_codex(
        self,
        _mock_which: MagicMock,
        mock_run: MagicMock,
        _mock_package_version: MagicMock,
        ...
        for result in (
            ...
            subprocess.CompletedProcess(
                args=[], returncode=0, stdout="unknown version\n", stderr=""
            ),
        ):
            with self.subTest(result=result):
                codex_pool._newer_system_codex_bin.cache_clear()
                mock_run.side_effect = result if isinstance(result, OSError) else None
                mock_run.return_value = None if isinstance(result, OSError) else result

                self.assertIsNone(codex_pool.app_server_config().codex_bin)
    ...
    def test_app_server_accepts_astra_system_codex_without_bundle_metadata(
        self,
        _mock_which: MagicMock,
        mock_run: MagicMock,
        _mock_package_version: MagicMock,
        ...
        self.assertEqual(
            codex_pool.app_server_config().codex_bin, "/usr/local/bin/codex"
        )
...
class ApprovalHandlerTests(TestCase):
    ...
    def test_mcp_tool_confirmation_uses_live_mode_and_wire_action(self) -> None:
        ...
        self.assertEqual(list(ApprovalRequest.objects.order_by("pk").values_list("decision", flat=True)),
                         ["accept", "decline"])
        self.assertFalse(ApprovalRequest.objects.filter(decision="").exists())
        self.assertEqual([event[0] for event in events], ["approval/resolved", "approval/resolved"])
        self.assertTrue(all(event[1]["automatic"] and event[1]["params"] == params for event in events))
        with patch("hitch.main.management.commands.codex_worker._current_approval_mode",
                   side_effect=["prompt_user", "approve_all"]):
            self.assertEqual(handler("mcpServer/elicitation/request", params)["action"], "accept")
        self.assertEqual(ApprovalRequest.objects.count(), 3)
        self.assertFalse(ApprovalRequest.objects.filter(decision="").exists())
        self.assertTrue(events[-1][1]["automatic"])
    ...
    def test_mcp_data_and_auth_elicitations_are_not_automatically_approved(self) -> None:
        ...
        self.assertEqual(ApprovalRequest.objects.filter(decision="decline").count(), 4)
        self.assertFalse(ApprovalRequest.objects.filter(decision="").exists())
