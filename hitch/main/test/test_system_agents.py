...
class SpecCriticWorkflowTests(TestCase):
    ...
    def test_pr_monitor_observation_clears_stale_ci_when_rollup_is_null(
        ...
        for call in mock_run.call_args_list:
            self.assertEqual(
                call.kwargs["timeout"],
                gh_cli._GH_PR_MONITOR_TIMEOUT_SECONDS,
            )
    ...
    @patch("hitch.main.workflows.pr_qa._spawn_pr_followup_monitor_run")
    @patch("hitch.main.workflows.pr_qa._spawn_pr_qa_run")
    def test_reconcile_routes_index_zero_workflow_turn(
        self,
        mock_spawn_qa: MagicMock,
        mock_spawn_monitor: MagicMock,
        mock_spawn_turn: MagicMock,
    ) -> None:
        # ``next_user_message_index`` is still 0 exactly when the spawner died
        # before saving the increment -- the dead-spawn shape the dual-index
        # match exists for. Skipping such workflows stranded the turn and let
        # the zombie recovery later block them with a misleading explanation.
        workflow = SystemWorkflow.objects.create(
            kind=SystemWorkflow.KIND_PR_QA,
            main_thread_id="main-thread",
            cwd="/repo",
    ...
    def test_qa_verdict_yields_to_concurrent_user_steering_claim(
        ...
        # its qa_review_revision bump with the stale snapshot.
        workflow = SystemWorkflow.objects.create(
            kind=SystemWorkflow.KIND_PR_QA,
            main_thread_id="main-thread",
            cwd="/repo",
        ...
        instance = _instance(
            ...
            events_path=_events_file(
                self,
                {"feedback": "Tighten the tests", "lgtm": False},
            ),
        )
        run = SystemAgentRun.objects.create(
            workflow=workflow,
...
class AutonomousGoalWorkflowTests(TestCase):
    ...
    def test_recover_stranded_candidate_spawn_drains_next_manual_queue_item(
        self, mock_spawn: MagicMock
    ) -> None:
        goal = self._autonomous_goal()
        workflow = self._stranded_autonomous_goal_workflow(
        ...
        queued_goal = AutonomousGoal.objects.create(
            ...
            title="Queued goal",
            goal="Start after recovery blocks the stranded head.",
        )
    ...
    def test_stop_fresh_no_run_spawn_gap_keeps_queue_waiting(
        ...
        active_goal = AutonomousGoal.objects.create(
            project=project,
            title="Active goal",
            goal="Its spawn is still in flight.",
        )
        queued_goal = AutonomousGoal.objects.create(
            project=project,
            title="Queued goal",
            goal="Should wait for the in-flight spawn gap.",
        )
        workflow = SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
            ...
            status=SystemWorkflow.STATUS_RUNNING,
            step=system_agents.STEP_AUTONOMOUS_GOAL_CANDIDATE_RUNNING,
            state={"autonomous_goal_id": active_goal.pk, "auto_proposal": False},
        )
        queued_workflow = SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
        ...
        stopped = autonomous_goals.stop_running_autonomous_goal_workflow(
            active_goal.pk, system_agents.AUTONOMOUS_GOAL_DELETED_ERROR
        )
    ...
    @patch("hitch.main.workflows.autonomous_goals.cleanup_managed_worktree_path")
    @patch("hitch.main.workflows.system_agents.codex_pool.interrupt_instance")
    def test_terminal_late_spawn_after_stop_drains_manual_queue(
        self,
        mock_interrupt: MagicMock,
        ...
        active_goal = AutonomousGoal.objects.create(
            project=project,
            title="Active goal",
            goal="Its spawn completes after the stop.",
        )
        queued_goal = AutonomousGoal.objects.create(
            project=project,
            title="Queued goal",
            goal="Should start after the terminal late spawn.",
        )
        workflow = SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
        ...
        run = SystemAgentRun.objects.create(
            ...
            instance=instance,
            status=SystemAgentRun.STATUS_RUNNING,
            input={"autonomous_goal_id": active_goal.pk},
        )
        mock_interrupt.return_value = instance

        with self.captureOnCommitCallbacks(execute=True):
            autonomous_goals._interrupt_spawned_autonomous_goal_run_if_inactive(run)
    ...
    @patch("hitch.main.workflows.system_agents.codex_pool.spawn_new_session")
    def test_manual_start_waits_when_stopped_autonomous_goal_worker_is_active(
        self, mock_spawn: MagicMock
    ) -> None:
        project = _make_project()
        worker_kinds = (
        ...
        for index, agent_kind in enumerate(worker_kinds, start=1):
            with self.subTest(agent_kind=agent_kind):
                SystemWorkflow.objects.all().delete()
                AutonomousGoal.objects.all().delete()
                mock_spawn.reset_mock()
                stopped_goal = AutonomousGoal.objects.create(
                    project=project,
                    title=f"Stopped goal {index}",
                    goal="Still has a live interrupted worker.",
                )
                stopped_workflow = SystemWorkflow.objects.create(
                    ...
                    step=system_agents.STEP_BLOCKED,
                    state={
                        "autonomous_goal_id": stopped_goal.pk,
                        "auto_proposal": False,
                    },
                )
                CodexInstance.objects.create(
                    pid=123 + index,
                    thread_id=f"stopped-thread-{index}",
                    cwd=project.repo_path,
                ...
                waiting_goal = AutonomousGoal.objects.create(
                    project=project,
                    title=f"Waiting goal {index}",
                    goal="Should not start while the old worker is active.",
                )
    ...
    def test_manual_start_drains_idle_existing_queue_before_returning_busy(
        self, mock_spawn: MagicMock
    ) -> None:
        project = _make_project()
        queued_goal = AutonomousGoal.objects.create(
            project=project,
            title="Queued goal",
            goal="Recover this idle queue item.",
        )
        waiting_goal = AutonomousGoal.objects.create(
            project=project,
            title="Waiting goal",
            goal="Should not jump ahead.",
        )
        queued_workflow = SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
    ...
    def test_idle_queued_workflow_makes_autonomous_goal_queue_busy(self) -> None:
        project = _make_project()
        queued_goal = AutonomousGoal.objects.create(
            project=project,
            title="Queued goal",
            goal="Occupy the global queue.",
        )
        SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
    ...
    def test_manual_start_runs_requested_goal_after_clearing_stale_queue(
        self, mock_spawn: MagicMock
    ) -> None:
        project = _make_project()
        stale_goal = AutonomousGoal.objects.create(
            project=project,
            title="Deleted queued goal",
            goal="This queued row should be cleared.",
            deleted_at=timezone.now(),
        )
        waiting_goal = AutonomousGoal.objects.create(
            project=project,
            title="Waiting goal",
            goal="Should start after stale queued work is blocked.",
        )
        stale_workflow = SystemWorkflow.objects.create(
            ...
            status=SystemWorkflow.STATUS_QUEUED,
            step=system_agents.STEP_AUTONOMOUS_GOAL_CANDIDATE_RUNNING,
            state={"autonomous_goal_id": stale_goal.pk, "manual_run_all_queue": True},
        )
    ...
    def test_manual_start_waits_behind_other_project_queued_work(
        ...
        waiting_goal = AutonomousGoal.objects.create(
            project=project,
            title="Waiting goal",
            goal="Should start instead of draining another project.",
        )
        other_workflow = SystemWorkflow.objects.create(
            ...
            status=SystemWorkflow.STATUS_QUEUED,
            step=system_agents.STEP_AUTONOMOUS_GOAL_CANDIDATE_RUNNING,
            state={"autonomous_goal_id": other_goal.pk, "manual_run_all_queue": True},
        )
    ...
    def test_candidate_no_proposal_drains_next_manual_queue_item(
        ...
        queued_goal = AutonomousGoal.objects.create(
            project=project,
            title="Improve docs",
            goal="Run after no-proposal completion.",
        )
        candidate_metadata = SessionMetadata.objects.create(
            thread_id="candidate-thread",
            cwd="/repo",
        ...
        instance = _instance(
            ...
            events_path=_events_file(
                ...
                {
                    "proposal": None,
                    "message": "No concrete increment was worth proposing.",
                    "next_steps_summary": "Try another area next.",
    ...
    def test_candidate_no_proposal_defers_manual_queue_drain_until_commit(
        self, mock_spawn: MagicMock
    ) -> None:
        project = _make_project()
        autonomous_goal = AutonomousGoal.objects.create(
    ...
    def test_stopping_running_goal_drains_next_manual_queue_item(
        ...
        SystemWorkflow.objects.filter(pk=running_workflow.pk).update(
    ...
    def test_reconcile_cancelled_terminal_ag_run_drains_manual_queue(
        self, mock_spawn: MagicMock
    ) -> None:
        project = _make_project()
        stopped_goal = AutonomousGoal.objects.create(
            project=project,
            title="Deleted goal",
            goal="Its terminal callback was missed.",
        )
        queued_goal = AutonomousGoal.objects.create(
            project=project,
            title="Queued goal",
            goal="Start after reconciliation.",
        )
        workflow = SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
    ...
    def test_reconcile_completed_resolution_ag_run_drains_manual_queue(
        self, mock_spawn: MagicMock
    ) -> None:
        project = _make_project()
        stopped_goal = AutonomousGoal.objects.create(
    ...
    def test_manual_queue_skips_deleted_head_and_starts_next_goal(
        self, mock_spawn: MagicMock
    ) -> None:
        project = _make_project()
        deleted_goal = AutonomousGoal.objects.create(
            project=project,
            title="Deleted goal",
            goal="Should be skipped.",
            deleted_at=datetime.now(UTC),
        )
        valid_goal = AutonomousGoal.objects.create(
            project=project,
            title="Valid goal",
            goal="Should start.",
        )
        deleted_workflow = SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
                deleted_goal.pk
            ),
            ...
            status=SystemWorkflow.STATUS_QUEUED,
            step=system_agents.STEP_AUTONOMOUS_GOAL_CANDIDATE_RUNNING,
            state={"autonomous_goal_id": deleted_goal.pk},
        )
        valid_workflow = SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
                valid_goal.pk
            ),
            ...
            status=SystemWorkflow.STATUS_QUEUED,
            step=system_agents.STEP_AUTONOMOUS_GOAL_CANDIDATE_RUNNING,
            state={"autonomous_goal_id": valid_goal.pk},
        )

        started = autonomous_goals._start_next_queued_autonomous_goal_workflow()

        deleted_workflow.refresh_from_db()
        valid_workflow.refresh_from_db()
        self.assertEqual(started, valid_workflow)
        self.assertEqual(deleted_workflow.status, SystemWorkflow.STATUS_BLOCKED)
        self.assertEqual(valid_workflow.status, SystemWorkflow.STATUS_RUNNING)
        mock_spawn.assert_called_once_with(valid_workflow, valid_goal)
    ...
    @patch("hitch.main.workflows.system_agents.codex_pool.spawn_new_session")
    def test_project_scoped_auto_proposals_do_not_drain_other_project_queue(
        self, mock_spawn: MagicMock, _mock_default_sha: MagicMock
    ) -> None:
        project = _make_project()
        other_project = _make_project(name="Other", repo_path="/other")
        ...
        other_goal = AutonomousGoal.objects.create(
            project=other_project,
            title="Other queued goal",
            goal="Should not start from a scoped run.",
        )
        other_workflow = SystemWorkflow.objects.create(
            kind=system_agents.AUTONOMOUS_GOAL_AGENT_KIND,
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
            ...
            status=SystemWorkflow.STATUS_QUEUED,
            step=system_agents.STEP_AUTONOMOUS_GOAL_CANDIDATE_RUNNING,
            state={"autonomous_goal_id": other_goal.pk},
        )
    ...
    def test_manual_run_all_continues_queue_after_spawn_blocks_head(
        self, mock_spawn: MagicMock
    ) -> None:
        project = _make_project()
        first_goal = AutonomousGoal.objects.create(
            project=project,
            title="Broken goal",
            goal="Blocks during spawn.",
        )
        second_goal = AutonomousGoal.objects.create(
            project=project,
            title="Next goal",
            goal="Should start after the block.",
        )

        def block_first_spawn(workflow: SystemWorkflow, _goal: AutonomousGoal) -> None:
            if workflow.state["autonomous_goal_id"] == first_goal.pk:
                workflow.status = SystemWorkflow.STATUS_BLOCKED
                workflow.state = {**workflow.state, "error": "spawn failed"}
                workflow.save(update_fields=["status", "state", "updated_at"])

        mock_spawn.side_effect = block_first_spawn

        autonomous_goals.start_autonomous_goal_workflows_or_queue(
            autonomous_goals=[first_goal, second_goal],
            use_worktrees=True,
        )

        first_workflow = SystemWorkflow.objects.get(
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
                first_goal.pk
            )
        )
        second_workflow = SystemWorkflow.objects.get(
            main_thread_id=autonomous_goals._autonomous_goal_main_thread_id(
                second_goal.pk
            )
        )
        self.assertEqual(first_workflow.status, SystemWorkflow.STATUS_BLOCKED)
        self.assertEqual(second_workflow.status, SystemWorkflow.STATUS_RUNNING)
        self.assertEqual(mock_spawn.call_count, 2)
    ...
    @patch(
        "hitch.main.workflows.autonomous_goals._start_next_queued_autonomous_goal_workflow",
        side_effect=OperationalError("database is locked"),
    )
    def test_auto_proposal_batch_survives_locked_pre_drain(
        ...
        self.assertEqual(started, 1)
        mock_drain.assert_called_once_with(project=project)
        mock_start.assert_called_once_with(goal.pk)
