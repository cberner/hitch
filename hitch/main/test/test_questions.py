                ...
                with TemporaryDirectory() as directory:
                    ...
                    path.write_text(json.dumps({"type": "event_msg", "payload": {
                        "type": "item_completed", "item": {
                            "type": "AgentMessage", "id": item["id"],
                            "content": [{"type": "Text", "text": "Question " * 10000}],
                            "delivery": "async", "questions": [{"title": "Question " * 10000}],
                        },
                    }}) + "\n")
                    oversized_preview = rollout.session_history_page(path)
                    assert oversized_preview is not None
                    histories.append(list(collapse_flat_entries(list(oversized_preview.flat_entries))))
                for entries in histories:
                    persisted = render_to_string("_session_entries.html", {"entries": entries})
                    loader = '<button data-history-loader data-next-url="/earlier">Load earlier messages</button>'
                    page.set_content(html.replace("<div data-live-root", persisted + loader +
                    ...
                    page.evaluate("html => window.earlierHistory = html",
                                  '<div data-history-page>' + persisted + '<p>Earlier history loaded</p></div>')
                    page.get_by_role("button", name="Load earlier messages").click()
                    page.get_by_text("Earlier history loaded", exact=True).wait_for()
                    self.assertEqual(page.locator("[data-history-loader]").count(), 0)
                    self.assertEqual(page.locator("[data-async-question-item-id]").count(), 0)
